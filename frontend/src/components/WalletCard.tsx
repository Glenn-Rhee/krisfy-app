...
const ListItem = ({ src, alt, width, title, description, onClick }: { src?: string, alt?: string, width?: number, title?: string, description?: string, onClick?: () => void }) => {
  return <li>
    <button onClick={onClick} className="flex items-start w-full active:scale-95 transition-all ease-in-out duration-50 cursor-pointer w-full bg-slate-700/50 p-4 rounded-xl border-1 border-gray-600 hover:border-gray-500 shadow-md hover:shadow-lg hover:shadow-gray-900">
